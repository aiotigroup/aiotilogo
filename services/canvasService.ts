...
/**
 * Phân tích cây DOM của chuỗi HTML đầu vào để tạo danh sách phân đoạn văn bản có style.
 */
function getStyledSegments(html: string, globalBold: boolean, globalItalic: boolean, globalUnderline: boolean): TextSegment[] {
  ...
  traverse(div, { b: globalBold, i: globalItalic, u: globalUnderline });
  return segments;
}

/**
 * Chia các phân đoạn thành các dòng văn bản, hỗ trợ xuống dòng (\n).
 */
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      if (textConfig.enabled && textConfig.content.trim()) {
        ...
        // Parse segments và chia dòng
        const segments = getStyledSegments(
          textConfig.content, 
        ...
        // Tính toán kích thước bao quanh cho từng dòng
        const lineMetrics = lines.map(line => {
          let width = 0;
          line.forEach(seg => {
            ctx.font = `${seg.italic ? 'italic' : 'normal'} ${seg.bold ? 'bold' : 'normal'} ${scaledFontSize}px ${textConfig.fontFamily}`;
